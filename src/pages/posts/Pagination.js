...
function Pagination({ perPage, total, current, limit }) {
	...
	const firstDigitsJSX = digits.slice(0, Math.min(digits.length, limit)).map((item, index) => (
		<NavLink to={`/posts${item}`} key={item}>
	...
	const currentDigitsJSX = digits.slice(Math.max(limit,current - limit), Math.min(digits.length - limit, current + limit - 1))
		.map((item, index) => (
			<NavLink to={`/posts${item}`} key={item}>
				<Button color={current === item ? 'primary' : 'inherit'} size={buttonSize}>
					{item}
